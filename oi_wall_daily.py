"""每日盘前 OI 墙报告：NVDA / GLD / SLV / SOXX → 图发邮箱（数据源：Yahoo Finance）"""
...
def plot_one(ax, symbol, spot, call_oi, put_oi):
    ...
    ax.set_title(f"{symbol}  spot={spot:.2f}", fontsize=11)
...
def build_report():
    ...
    for ax, sym in zip(axes.flat, TICKERS):
        try:
            spot, call_oi, put_oi = fetch_oi(sym)
            plot_one(ax, sym, spot, call_oi, put_oi)
...
def send_email(image_buf, summary_text):
    msg = MIMEMultipart()
    msg["Subject"] = f"[OI 墙] {datetime.now().strftime('%Y-%m-%d')} NVDA/GLD/SLV/SOXX"
